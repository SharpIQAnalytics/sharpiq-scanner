...
for game in games:
    ...
    cursor.execute('''
                    INSERT INTO games
                    (date,kickoff_utc,home_team,away_team,home_odds,away_odds,best_price_home,best_price_away,best_home_bookmaker,best_away_bookmaker)
                    VALUES(?,?,?,?,?,?,?,?,?,?) 
                    ON CONFLICT(date,home_team,away_team)
                    DO UPDATE SET
                    kickoff_utc=excluded.kickoff_utc,home_odds=excluded.home_odds,away_odds=excluded.away_odds,best_price_home=excluded.best_price_home,best_price_away=excluded.best_price_away,best_home_bookmaker=excluded.best_home_bookmaker,best_away_bookmaker=excluded.best_away_bookmaker
                   ''',
                    (date,kickoff_utc,game['home_team'],game['away_team'],sportsbet_home,sportsbet_away,best_home,best_away,best_home_bookmaker,best_away_bookmaker))
conn.commit()
print("Games updated")